...
func Plotting(ch chan float64) {
	...
	gnuplotArgs := `set terminal pngcairo size 800,600 enhanced font 'Verdana,12';
	 set output 'pid_plot.png';
	 set grid lw 1;
	 set xlabel 'Time (s)';
	 set ylabel 'Controlled Variable';
	 set style line 1 lc 1 lw 2 ;
	 plot 'plot_data.txt' using 1:2 with lines ls 1 title 'PID controller test';`
